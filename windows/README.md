...
2. **Left-click** the icon. ClipRef saves whatever's on the clipboard — text as a `.txt`, an image
   ...
   checkmark with a sound. Your clipboard now holds an `@`-path to that file.
3. Switch to Claude Code and press **Ctrl+V**. The pasted `@…` becomes a file reference.
...
- **Double-clicking is safe:** if your clipboard already holds an `@`-reference to a Windows path,
  ClipRef leaves it alone instead of saving the reference into a new file.
